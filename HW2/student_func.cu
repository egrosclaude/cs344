...
//	__shared__ s_inputChannel[TILE_SIZE][TILE_SIZE];
    ...
    if((y < numRows) && (x < numCols)) {
        ...
        float v = 0.f;
        // x, y thread coords
        // i, j coords in filter
		// vx, vy coords in input under filter
	    
	    for(int j = 0; j < filterWidth; j++) {
   			int vx = x - filterWidth/2 + j;
   			vx = max (min (vx, numCols-1), 0);
   			for(int i = 0; i < filterWidth; i++) {
        			int vy = y - filterWidth/2 + i;
		   			vy = max (min (vy, numRows-1), 0);
       				v += (float)inputChannel[vy * numCols + vx] * filter[i * filterWidth + j];
       		}
        }
        outputChannel[y * numCols + x] = v;
	}
                      ...
                      unsigned char* const blueChannel)
{
  ...
  // }
    ...
    if((y < numRows) && (x < numCols)) {
        int o = y * numCols + x;
        redChannel[o] =  inputImageRGBA[o].x;
        greenChannel[o] =  inputImageRGBA[o].y;
       	blueChannel[o] =  inputImageRGBA[o].z;
    }
                        ...
                        const int filterWidth)
{
  ...
  //from the image size and and block size.
  	const dim3 gridSize(ceil((float)numCols / 32), ceil((float)numCols / 32), 1);
